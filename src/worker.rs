...
use polybot_sdk_v2::clob::types::{Side, SignatureType, OrderType, OpenOrderStatusType, Amount, TickSize};
...
pub use polybot_sdk_v2::error::{Error, Status, Kind, Validation};
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (window_ts, order_id) in active_tracked_orders {
                    ...
                    if let Ok(order_info) = get_order_status(&order_id).await {
                        ...
                        // 2. Map target internal statuses based on our new OpenOrderStatusType enum variations
                        let target_status = match order_info.status {
                            OpenOrderStatusType::Live => {
                            ...
                            OpenOrderStatusType::Canceled 
                            | OpenOrderStatusType::CanceledMarketResolved 
                            | OpenOrderStatusType::Invalid => {
            ...
            match cmd {
                ...
                UiCommand::PlaceLimit { side, token, price, size, window_ts } => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let endpoint_path = format!("/order/{}", slug);
...
async fn place_order_limit(
    ...
    let creds = timed!("get api creds", { get_or_fetch_api_creds(private_key.clone(), host.clone()).await? });
...
async fn get_order_status(order_id: &str) -> anyhow::Result<OpenOrderResponse> {
    let private_key = std::env::var("PRIVATE_KEY_VAR")?;
    let host = std::env::var("CLOB_API_URL").unwrap_or_else(|_| "https://clob.polymarket.com".into());
    let deposit_wallet = Address::from_str(&std::env::var("DEPOSIT_WALLET")?)?;
...
async fn cancel_order(order_id: &str) -> anyhow::Result<CancelOrdersResponse> {
    let private_key = std::env::var("PRIVATE_KEY_VAR")?;
    let host = std::env::var("CLOB_API_URL").unwrap_or_else(|_| "https://clob.polymarket.com".into());
    let deposit_wallet = Address::from_str(&std::env::var("DEPOSIT_WALLET")?)?;
    ...
    tracing::info!("Order response: {:?}", resp);

    Ok(resp)
}

async fn cancel_all_orders() -> anyhow::Result<CancelOrdersResponse> {
    let private_key = std::env::var("PRIVATE_KEY_VAR")?;
    let host = std::env::var("CLOB_API_URL").unwrap_or_else(|_| "https://clob.polymarket.com".into());
    let deposit_wallet = Address::from_str(&std::env::var("DEPOSIT_WALLET")?)?;
